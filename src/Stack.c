...
static int Stack_Resize_Up (struct Stack *stack_ptr)
{
    ...
    char *r_boundary = raw_ptr + CANARY_SZ + stack_ptr->capacity * ELEM_SZ;

    memset (r_boundary, 0, CANARY_SZ + stack_ptr->capacity * ELEM_SZ * (MULTIPLIER - 1));

    #if SECURITY_LEVEL == 1 || SECURITY_LEVEL == 2
    *(canary_t *)(r_boundary + stack_ptr->capacity * ELEM_SZ * (MULTIPLIER - 1)) = DATA_R_CANARY;
    #endif
